...
**Sessions 1, 3A, 2, 4, 5, 6 and 7 complete — application shell, a fully built and fully dormant AI
layer, the complete law dataset with the Law Converter built over it, the complete pay dataset with
the Pay & Allowances Calculator built over it, and the CSMOP 2022 templates, Rajbhasha terms and
drafting engine that the Drafting Studio will be built on. Its UI, the Trainer and Utilities to go.**
...
| i18n            | 462 keys × {en, hi}, typed via module augmentation, `fallbackLng: false`. Nav labels live in `src/lib/nav.ts` as `{ en, hi }`. The `pay.*` block is now the largest single one (184 keys), the `law.*` block next; the AI consent copy is still the largest block of prose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Modules         | **Law Converter and Pay & Allowances are built** (rows above and below). **Drafting has its data and its engine but no UI yet** — `/draft` is still a placeholder page. Trainer and Utilities are placeholder pages too (`ModulePlaceholder`: masthead + file-tabbed card + disclaimer)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Drafting engine | `src/lib/drafting/` — pure, takes the template as an argument. `render(template, values, 'en' \| 'hi' \| 'bilingual')` → a structured model (header, refLine, subject, paras[], closing, signature, copyTo[], enclosures[]) plus a plain-text serialiser that wraps and aligns; required-field, date-format and unknown-option validation; auto paragraph numbering; auto `Encl.: as above (n)`. `checklist.ts` implements thirteen rule kinds and throws on an unimplemented one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
- **Seven of the fourteen forms are not in CSMOP at all** — circular, leave application,
  ...
  in both languages. The test suite fails a template that claims to be unverified without both. When
  the Drafting UI is built, a `verify: true` template owes the reader the same marigold banner the
  Law Converter shows for curated Hindi.
