...
**Sessions 1, 3A, 2, 4 and 5 complete — application shell, a fully built and fully dormant AI layer,
the complete law dataset for all three codes with the Law Converter built over it, and the complete
pay dataset waiting for the calculator that will read it. Four modules to go.**
...
| i18n          | 292 keys × {en, hi}, typed via module augmentation, `fallbackLng: false`. Nav labels live in `src/lib/nav.ts` as `{ en, hi }`. The `law.*` block is the largest single one; the AI consent copy is the largest block of prose                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Modules       | **Law Converter is built** (row above). **Pay & Allowances has its data but no screen yet** — `data/pay` is complete and tested, `src/modules/pay` is a placeholder page plus the zod schema. Trainer, Drafting and Utilities are placeholder pages (`ModulePlaceholder`: masthead + file-tabbed card + disclaimer)                                                                                                                                                                                                                                                                                                                                                                          |
...
| Voice search  | Dictate a query into the Law Converter. Recognised **on the device** or refused — `processLocally` is required and there is no cloud fallback, so there is no consent gate and nothing to warn about (ADR-017, superseding ADR-014). `src/lib/voice.ts` is the only module allowed to touch the speech API, by a file-scoped eslint exception; dynamically imported on the first press; `hi-IN` / `en-IN`; one utterance, never continuous                                                                                                                                                                                                                                                   |
...
| Tests         | 911 unit across 39 files (5 skip without a build) + 59 Playwright e2e, the law ones run at four viewports. The law suites read the committed `data/law/*.json` off disk — dataset coverage, ranking against a 50-query bilingual acceptance set (`tests/fixtures/law-search.ts`), every "What's new" bullet resolved and its claim checked. `tests/pay-data.test.ts` reads `data/pay/*.json` off disk the same way — 95 assertions, every one confirmed to fail against a deliberately corrupted copy (ADR-016 addendum). e2e adds an offline section lookup with no network at all, a real-input privacy run, the keyboard path, and axe over an open section card × 2 languages × 2 themes |
...
- **Two modules are confined by name in `eslint.config.js`**: `src/ai/providers/wire.ts` is the only one
  that may call `fetch` (and only after AI consent), and `src/lib/voice.ts` the only one that may touch
  the speech API — where it must set `processLocally`, because that flag is the whole reason voice
  search needs no consent (ADR-017). `tests/no-external-urls.test.ts` asserts each count independently
  of the lint rule. Adding a third means an ADR, not a disable comment.
