...
- Equal bilingual content: every data record stores { en, hi } objects, not flat strings. Missing hi is a
  ...
  --coral #F43F5E low band, --destructive #C92A1E (spec said #DA3125 — 4.48:1 as text, ADR-010),
  --muted #F2F5FA / --muted-foreground #667085, --border #DFE5EF (decorative) but --input #7B8BA8 split off
  for the 3:1 control floor (ADR-007), --secondary #E4EAF4, --accent #DBE4F8 / #123086,
  --sidebar #F1F5FB (spec said #EFF3FA — 4.47:1 with --muted-foreground).
  Dark: --background #061225, --card #102542, --foreground #E8EDF5, --primary #6E9BFF, --action #F7C873 on
  navy, --muted-foreground #A9B7CC, --tulsi #34D399, --coral #FB7185, --destructive #FB8378 (spec said
  #F2685C — 4.40:1 on --muted), --border #22344F and --input #6B80A4 as solid hex, not oklch alpha.
  ALL FIVE DEVIATIONS ARE MEASURED AND RECORDED IN ADR-010; src/styles/tokens.test.ts is what caught them.
  ...
  Signature element (DECIDED, Session 1B): the "file tab" — a 3px --marigold index tab on the top edge of the
  active module card (SectionCard active), the same 3px rule on the sidebar's left edge and the tab bar's top
  edge for the active destination, and the SectionNumber chip set in tabular Inter on --muted. One tab per
  screen. No gauge (that is Neev's). Full reference: .claude/skills/frontend-design/SKILL.md.
...
| i18n          | 61 keys × {en, hi}, typed via module augmentation, `fallbackLng: false`. Nav labels live in `src/lib/nav.ts` as `{ en, hi }`                                                                                                                                                               |
...
| Tests         | 177 unit across 10 files (2 skip without a build) + 13 Playwright e2e (offline shell, zero third-party requests, axe × 2 languages × 2 themes, light-default theme, real rendered fonts)                                                                                                   |
