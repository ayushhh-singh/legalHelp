 ...
 *
 * `/study/practise/review?act=<actId>` (`/study/practise/review?act=…` before ADR-046) already
 * existed informally before this file:
...
export function toTrainerTopicHref(actId: string): string {
  return `/study/practise/review?act=${encodeURIComponent(actId)}`
}
