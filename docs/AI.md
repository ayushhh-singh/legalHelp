...
## 7D. The intake-analysis and modify-by-instruction agents

Session 31, ADR-043. Two agents, in the Drafting Studio, and the thing worth
knowing about both is what the app does around them rather than what they do.

### `intake-analyse` is the SECOND pass over a letter

`src/lib/drafting/intake.ts` has already read the letter — number, date, receipt
date, subject, urgency, enclosure count, cited provisions and the sentences that
ask for something — with no model, on every device, with AI off. The officer has
already corrected every chip. **Nothing this agent returns may contradict it**,
and the deadline is the worked example: if the extractor read one off a labelled
sentence and the model wrote a different one, the extractor's is what is shown
and the disagreement is reported. The model may have done arithmetic on a period
("within 15 days"), which the prompt forbids and which nothing else would catch.

What a model adds is what extraction cannot do: a bilingual summary, a reading of
what is being asked, the risks in answering badly, and which of the forty-three
forms answers it. That is a small enough job that this file is short, which is
the ordering §13 asks for arriving at its natural conclusion.

**One rule fails the run closed: a cited provision must be backed by a snippet it
was shown.** A `citedProvisions` entry naming a snippet id it was not given ends
the run — it is NOT dropped and reported the way `parseFieldValues` drops an
unknown field. The difference is what the output IS: an unknown field is a value
nobody asked for and the rest of the draft is still usable, while this whole
output is one claim about what the letter says, and a provision the letter never
cited inside that claim is an officer answering the wrong point.

The citation an officer sees is **re-derived from the snippet**, never the string
the model wrote — ADR-035's rule, applied again.

**The letter is what leaves the device**, and the confirmation says so in those
words before the first run: "the whole letter goes to the AI provider you
configured in Settings". `screenOutbound` — the same screen the drafting agent
uses — refuses a letter naming classified or departmental material BEFORE the
provider is constructed, and `intake.test.ts` counts `MockProvider.calls` to
prove it.

### `doc-modify` returns a proposal, and the scope is enforced in code

The model is handed the officer's document and an instruction and returns a whole
new body. That body never reaches anybody: it is parsed against `bodySchema`'s
closed node list, read back through `readDoc`, diffed block by block and then
word by word, marked against the checklist, and returned as a list of decisions
with accept and reject on each. Nothing is applied until the officer accepts, and
an accepted set creates one version.

**A scoped instruction is enforced by `restrictToBlocks`, not by the prompt.**
The prompt asks for it too, and the prompt is not the enforcement — a persona
instruction is not a check. Every change outside the selected block is discarded
and REPORTED, so the panel says "two changes outside the paragraph you selected
were discarded" rather than silently narrowing the result. `modify.test.ts` hands
a mock a document with an out-of-scope block changed and asserts on what comes
back.

**The checklist is evaluated over the result**, never claimed by the model —
ADR-032 §1, again. A rewrite that reads better and breaks `paraNumbering` is
reported and still offered, which is the OPPOSITE of `runDraftingAgent`'s
revision rule. The difference is who decides: this is a set of accept/reject
controls, so the officer can take the two changes they wanted and leave the one
that broke the numbering, and throwing the whole thing away would take the good
changes with it.

**The rationale may not name a paragraph number.** Paragraph numbers are
generated by position, so accepting an insertion renumbers everything below it
and the rationale is then about a paragraph the officer never touched. It would
also fail the run: `context.ts#NUMBERED_PROVISION_PATTERN` reads `paragraph N` as
a claim about a provision, which is `docs/DATA-GAPS.md` #60's wall from a new
direction. The prompt asks for opening words instead, and `modify.test.ts`
asserts the failure so it has a name if a future prompt edit loses that sentence.

### Both passes are toolless, so `groundedRequired` is off at the call site

ADR-035 §1's shape, and the fifth and sixth agents to take it. `TASK_DEFAULTS`
stays `true` for both so a second, tool-calling pass added later is grounded by
default rather than inheriting an exemption nobody chose. What replaces it is
listed in ADR-043 §4 and every item of it is in code.

### The per-template instruction is context, never cached prefix

`src/lib/drafting/instructions.ts#buildInstruction` derives a form-specific
instruction from the template's own `person`, `salutation`, `subscription`,
`urgencyAllowed`, layout order, CSMOP paragraphs and `must` checklist items. It
varies per document, per language and per field value, so it goes in a PLATFORM
CONTEXT snippet — a `buildSystem({ instructions })` block is cached and must be
the same bytes on every request an agent makes.

Every value it carries is sanitised there and nowhere else: control characters
and bidi overrides removed, and a 5,000-character cap per field with the
truncation reported. There is no path from a field value into a prompt that
skips it.
