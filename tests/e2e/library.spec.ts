...
test('reads three units offline and comes back to the third after a reload', async ({ page, context }) => {
  ...
  await expect(page.getByRole('heading', { level: 2, name: t('en', 'library.title') })).toBeVisible()
  ...
  await expect(page).toHaveURL(new RegExp(`/study/read/${WORK}$`))
  ...
  await page
    ...
    .click()
  await expect(page).toHaveURL(new RegExp(`/study/read/${WORK}/${UNITS[0]}$`))
  ...
  for (const unitId of UNITS.slice(1)) {
    ...
    await expect(page).toHaveURL(new RegExp(`/study/read/${WORK}/${unitId}$`))
  }
  ...
  await resume.click()
  await expect(page).toHaveURL(new RegExp(`/study/read/${WORK}/${UNITS[2]}$`))
})
...
test('searches inside a work and opens the hit', async ({ page }) => {
  ...
  await hit.click()
  await expect(page).toHaveURL(new RegExp(`/study/read/${WORK}/ccs-conduct-3$`))
})

test('moves between units with j and k, and not while typing', async ({ page }) => {
  await page.goto(`/study/read/${WORK}/${UNITS[1]}`)
  await expect(page.getByRole('heading', { level: 1 })).toBeVisible()
  ...
  await expect(page).toHaveURL(new RegExp(`/study/read/${WORK}/${UNITS[1]}$`))
...
test('is reachable in Hindi, from the tab bar on a phone and the sidebar on a desktop', async ({ page }) => {
  await page.goto('/settings/ai')
  await setLanguage(page, 'hi')

  await page.goto('/study/read')
  await expect(page.getByRole('heading', { level: 2, name: t('hi', 'library.title') })).toBeVisible()
  ...
  await page.goto('/study/read/fr-sr')
